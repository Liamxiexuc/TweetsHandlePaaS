...
public class Worker2 {	
	public static void main(String[] args) throws Exception {
		...
		try {
				...
				workerSocket=new Socket("127.0.0.1", 9004);
				System.out.println("Tweet Worker2 Started ....");
				Worker2TweetThread workerTweetThread = new Worker2TweetThread(workerSocket, map); //send  the request to a separate thread
				workerTweetThread.start();
				
				querySocket = new Socket("127.0.0.1", 9002);
				System.out.println("Query Worker2 Started ....");

				Worker2QueryThread workerQueryThread = new Worker2QueryThread(querySocket, map); //send  the request to a separate thread
...
class Worker2TweetThread extends Thread {
	Socket tweetSocket;
	Map<String,String> map;

	Worker2TweetThread(Socket inSocket, Map<String,String> inMap){
...
class Worker2QueryThread extends Thread {
	Socket querySocket;
	Map<String,String> map;
	static OperatingSystemMXBean operatingSystemMXBean = ManagementFactory.getOperatingSystemMXBean();

	Worker2QueryThread(Socket inSocket, Map<String,String> inMap){
